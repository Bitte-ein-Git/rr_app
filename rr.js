...
const close_report_modal = () => { toggle_modal('report-modal'); el('report-form').reset(); el('rr-report-player-display').innerHTML = ''; el('rr-report-status').style.display = 'none'; el('rr-other-description-group').style.display = 'none'; el('rr-additional-details-group').style.display = 'none'; el('rr-report-otherDescription').required = false; }
const show_report_modal = (fc, miiName) => {
    ...
    let miiSrc = './assets/loading.gif'; let miiKey = null;
    if (activePlayer && activePlayer.mii && activePlayer.mii.length > 0 && activePlayer.mii[0].data) { const rawMiiData = activePlayer.mii[0].data; const cachedUrl = get_cached_mii_image(rawMiiData); if (cachedUrl) { miiSrc = format_mii_src(cachedUrl); } else { miiKey = rawMiiData; } } else { const cachedMii = get_cached_mii_image(fc); if (cachedMii) { miiSrc = format_mii_src(cachedMii); } else { miiKey = fc; } }
    const miiDisplay = el('rr-report-player-display'); miiDisplay.innerHTML = `<img class="player-mii-large" src="${miiSrc}" alt="Mii" ${miiKey ? `mii-data="${miiKey}"` : ''}><div><div class="player-name">${handle_mii_name(miiName)}</div><div class="player-fc">${fc.replace(/[^0-9]/g, '').replace(/(\d{4})(?=\d)/g, '$1-')}</div></div>`;
    if (miiKey) { if (activePlayer && activePlayer.mii && activePlayer.mii[0].data) fetch_mii_images([miiKey]); else fetch_miis_by_fc([miiKey]); }
    el('rr-report-fc-hidden').value = fc; el('rr-report-miiName-hidden').value = miiName; toggle_modal('report-modal');
}

function get_cached_item(key, expireTime) {
    const item = localStorage.getItem(key); if (!item) return null;
    try { const [data, timestamp] = JSON.parse(item); if (Date.now() - timestamp > expireTime) { localStorage.removeItem(key); return null; } return { data, timestamp }; } catch (e) { localStorage.removeItem(key); return null; }
}
...
function clear_expired_cache() {
    let r = []; for (let i = 0; i < localStorage.length; i++) { const k = localStorage.key(i); if (k.startsWith("mii_img_")) get_cached_mii_image(k.replace("mii_img_", "")); else if (k === 'userProfileData') get_cached_item(k, PROFILE_EXPIRE_TIME); else if (k.startsWith("discord_")) get_cached_item(k, PROFILE_EXPIRE_TIME * 7); }
}
...
async function show_player_info_modal(fc) {
    ...
    const displayData = { FC: fc, name: displayName, lastSeen: activePlayer ? new Date().toISOString() : null, vr: "-", rank: "-", activeRank: "-", vrStats: { last24Hours: 0, lastWeek: 0, lastMonth: 0 } };
    mc.innerHTML = _get_player_profile_html(displayData, miiSrc, miiKey, false, true); 
    if (miiKey) { if (activePlayer && activePlayer.mii && activePlayer.mii.length > 0 && activePlayer.mii[0].data === miiKey) { fetch_mii_images([miiKey]); } else { fetch_miis_by_fc([miiKey]); } }
    load_discord_for_profile(fc, mc); 
    try { const r = await fetch(`${API_BASE_URL}?info=${fc}`); if (!r.ok) throw new Error(`API error: ${r.status}`); const d = await r.json();
        if (!activePlayer && !get_cached_mii_image(fc) && d.miiImageBase64) { const cachedFromProfile = get_cached_mii_image(d.miiImageBase64); if (cachedFromProfile) { miiSrc = format_mii_src(cachedFromProfile); mc.innerHTML = _get_player_profile_html(d, miiSrc, null, false, false); set_cached_mii_image(fc, cachedFromProfile, d.miiImageBase64); } else { fetch_mii_images([d.miiImageBase64]); mc.innerHTML = _get_player_profile_html(d, './assets/loading.gif', d.miiImageBase64, false, false); } } else { const currentImg = mc.querySelector('.player-mii-large'); const currentSrc = currentImg ? currentImg.src : miiSrc; mc.innerHTML = _get_player_profile_html(d, currentSrc, miiKey, false, false); } load_discord_for_profile(fc, mc); 
    } catch (e) { console.error("Error fetching player info:", e); const loadingEl = mc.querySelector('.player-info-loading-text'); if(loadingEl) loadingEl.remove(); }
}
...
function load_favorites() { const f = localStorage.getItem("favorites"); let favs = f ? JSON.parse(f) : []; if (favs.length > 0 && typeof favs[0].nickname === 'undefined') { FAVORITES = favs.map(oldF => ({ fc: oldF.fc, name: oldF.name, nickname: null })); save_favorites(); } else { FAVORITES = favs; } }
...
async function add_favorite(fc, name = null, e) { 
    ...
    if (!el('favorites-modal').classList.contains('hidden')) render_favorites_list(); 
...
async function search_player_for_fav() { const q = el('fav-search-input').value.trim(), rc = el('fav-search-results'); if (!q) { rc.innerHTML = '<p>Enter name or FC.</p>'; return; } rc.innerHTML = 'Searching...'; try { const r = await fetch(`${API_BASE_URL}?find=${encodeURIComponent(q)}`); if (!r.ok) throw new Error(`API error: ${r.status}`); const p = await r.json(); if (!p || p.length === 0) { rc.innerHTML = '<p>No players found.</p>'; return; } let mii_fcs = []; let h = ''; p.forEach(pl => { let mk = null; const cachedMii = get_cached_mii_image(pl.FC); let ms = cachedMii ? format_mii_src(cachedMii) : './assets/loading.gif'; if (!cachedMii) { mk = pl.FC; mii_fcs.push(mk); } if (is_favorite(pl.FC)) return; h += `<div class="player-row search-result-row" onclick="show_player_info_modal('${pl.FC}')"><img class="player-mii" src="${ms}" alt="Mii" ${mk ? `mii-data="${mk}"` : ''}><div class="player-info"> <div class="player-name">${handle_mii_name(pl.name)}</div> <div class="player-fc">${pl.FC}</div> </div><button class="select-profile-btn" style="background: #4CAF50;" onclick="add_favorite('${pl.FC}', decodeURIComponent('${encodeURIComponent(pl.name)}'), event)">+ Add</button></div>`; }); rc.innerHTML = h || '<p>All found players are already favorites.</p>'; if (mii_fcs.length > 0) fetch_miis_by_fc(mii_fcs); } catch (e) { rc.innerHTML = `<p style="color:red;">Search failed.</p>`; } }
async function render_favorites_list() { const lc = el('favorites-list'); if (FAVORITES.length === 0) { lc.innerHTML = '<p>No favorites added yet.</p>'; return; } lc.innerHTML = 'Loading favorites...'; let mii_fcs = []; let h = ''; for (const f of FAVORITES) { let mk = null; let ms = './assets/loading.gif'; const cachedMii = get_cached_mii_image(f.fc); if (cachedMii) { ms = format_mii_src(cachedMii); } else { mk = f.fc; mii_fcs.push(mk); } const displayName = get_display_name(f.fc, f.name); const nickText = (f.nickname) ? "Edit Nickname" : "Set Nickname"; const miiAttr = mk ? `mii-data="${mk}"` : ''; h += `<div class="player-row favorite-row"><img class="player-mii" src="${ms}" alt="Mii" ${miiAttr}><div class="player-info" onclick="show_player_info_modal('${f.fc}')"><div class="player-name">${handle_mii_name(displayName)}</div><div class="player-fc">${f.fc}</div></div><button class="edit-nickname-btn" onclick="show_nickname_modal('${f.fc}', '${encodeURIComponent(f.name)}', event)" title="${nickText}">✏️</button><button class="remove-favorite-btn" onclick="remove_favorite('${f.fc}', true, event)" title="Remove">X</button></div>`; } lc.innerHTML = h; if (mii_fcs.length > 0) fetch_miis_by_fc(mii_fcs); }
...
async function search_user() { const q = el('user-search-input').value.trim(), rc = el('user-search-results'); if (!q) { rc.innerHTML = '<p>Enter name or FC.</p>'; return; } rc.innerHTML = ''; let a = 0, mA = 2, s = false; while (a < mA && !s) { try { const r = await fetch(`${API_BASE_URL}?find=${encodeURIComponent(q)}`); if (!r.ok) throw new Error(`API error: ${r.status}`); const p = await r.json(); s = true; if (!p || p.length === 0) { rc.innerHTML = '<p>No players found.</p>'; return; } let mii_fcs = []; let h = ''; p.forEach((pl) => { let mk = null; const cachedMii = get_cached_mii_image(pl.FC); let ms = cachedMii ? format_mii_src(cachedMii) : './assets/loading.gif'; if (!cachedMii) { mk = pl.FC; mii_fcs.push(mk); } h += `<div class="player-row search-result-row" onclick="set_user_profile('${pl.FC}', '${encodeURIComponent(pl.name)}')"><img class="player-mii" src="${ms}" alt="Mii" ${mk ? `mii-data="${mk}"` : ''}><div class="player-info"> <div class="player-name">${handle_mii_name(pl.name)}</div> <div class="player-fc">${pl.FC}</div> </div><button class="select-profile-btn">Select</button></div>`; }); rc.innerHTML = h; if (mii_fcs.length > 0) fetch_miis_by_fc(mii_fcs); } catch (e) { a++; console.error(`Search attempt ${a} failed:`, e); if (a >= mA) rc.innerHTML = `<p style="color:red;">Search failed.</p>`; else await new Promise(res => setTimeout(res, 500)); } } }
...
function _get_player_profile_html(d, miiSrc, miiKey, isUserProfile = false, isPlaceholder = false) {
    ...
    return `<div class="player-info-modal-header"><img class="player-mii-large" src="${miiSrc}" alt="Mii" ${miiAttr}><div><div class="player-name">${handle_mii_name(profileName)}</div><div class="player-fc">${d.FC}</div></div></div><div class="player-info-modal-lastseen"> <div>Last Seen: ${format_last_seen(d.lastSeen)}</div>${isPlaceholder ? '<div class="player-info-loading-text">Loading profile...</div>' : ''}</div><div class="player-info-modal-stats-grid"><div class="stat-item"> <span class="stat-label">Rank</span> <span class="stat-value">${d.rank ? `#${d.rank}` : '-'}</span> </div><div class="stat-item"> <span class="stat-label">VR</span> <span class="stat-value">${val(d.vr)}</span> </div></div><h3 class="vr-stats-heading">VR Stats</h3><div class="player-info-modal-stats-grid"><div class="stat-item"> <span class="stat-label">24h</span> <span class="stat-value ${vrClass(vr24)}">${formatVRStat(vr24)}</span> </div><div class="stat-item"> <span class="stat-label">7d</span> <span class="stat-value ${vrClass(vr7)}">${formatVRStat(vr7)}</span> </div><div class="stat-item"> <span class="stat-label">30d</span> <span class="stat-value ${vrClass(vr30)}">${formatVRStat(vr30)}</span> </div></div><div class="player-info-modal-buttons"><button class="modal-close-btn visit-btn" onclick="window.open('https://rwfc.net/player/${d.FC}', '_blank')"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="visit-btn-icon"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg> Visit rwfc.net</button>${!isUserProfile ? `<button class="modal-close-btn report-btn" onclick="hide_player_info_modal(); show_report_modal('${d.FC}', '${d.name}')">🚩 Report Player</button>` : ''}</div><div class="player-info-modal-active-rank"> Active Rank: ${d.activeRank ? `#${d.activeRank}` : '-'} </div>${isUserProfile ? '<div id="user-profile-cache-status" class="player-info-modal-cache-status"></div>' : ''}`;
}
...
async function render_user_profile() {
    ...
    if (cached) {
        ...
        if (mk) fetch_miis_by_fc([mk]); load_discord_for_profile(USER_PROFILE.fc, dc);
    } else { dc.innerHTML = 'Loading...'; }
    try {
        const r = await fetch(`${API_BASE_URL}?info=${USER_PROFILE.fc}`); if (!r.ok) throw new Error(`API error: ${r.status}`); const d = await r.json(); set_cached_item(`profile_${USER_PROFILE.fc}`, d);
        let mk = null; let ms = './assets/loading.gif'; let useBatch = false;
        let cachedMii = get_cached_mii_image(d.FC);
        if (!cachedMii && d.miiImageBase64) { cachedMii = get_cached_mii_image(d.miiImageBase64); }
        if (cachedMii) { ms = format_mii_src(cachedMii); } else if (d.miiImageBase64) { mk = d.miiImageBase64; ms = './assets/loading.gif'; } else { mk = d.FC; useBatch = true; }
        ...
        load_discord_for_profile(USER_PROFILE.fc, dc); if (mk) { if (useBatch) fetch_miis_by_fc([mk]); else fetch_mii_images([mk]); }
    } catch (e) { console.error("Error fetching user profile:", e); if (!cached) dc.innerHTML = `<p style="color:red;">Error loading profile.</p>`; }
 }
...
function on_history_change() {
    const val = el("history-input").value;
    if(el("history-input-main")) el("history-input-main").value = val;
    if (HISTORY_TIMER) clearTimeout(HISTORY_TIMER); HISTORY_TIMER = setTimeout(change_history, 1000);
}

function on_history_main_change() {
    const val = el("history-input-main").value;
    if(el("history-input")) el("history-input").value = val;
    if (HISTORY_TIMER) clearTimeout(HISTORY_TIMER); HISTORY_TIMER = setTimeout(change_history, 1000);
}

function add_history(m) { 
    let i=el("history-input"),t=new Date(i.value).getTime(); t+=m*60*1000; let o=new Date().getTimezoneOffset()*60000; t-=o; 
    const newVal = new Date(t).toISOString().slice(0,16);
    i.value = newVal;
    if(el("history-input-main")) el("history-input-main").value = newVal;
    on_history_change();
}
...
// ... (Room Splitting/Prioritization/Uptime/HeaderStats logic) ...
...
function start_room_detail_flipper() {
    ...
    const show_avg_vr = el("avg-vr-checkbox").checked; document.querySelectorAll('.room-detail-flipper').forEach(e => { if(e.dataset.players) e.innerHTML = e.dataset.players; e.style.opacity = 1; });
    if (!show_avg_vr) return;
    const u = () => { if (!el("avg-vr-checkbox").checked) { if (ROOM_DETAIL_FLIPPER_TIMER) clearInterval(ROOM_DETAIL_FLIPPER_TIMER); document.querySelectorAll('.room-detail-flipper').forEach(e => { if(e.dataset.players) e.innerHTML = e.dataset.players; e.style.opacity = 1; }); return; } ROOM_DETAIL_STATE = ROOM_DETAIL_STATE === 'players' ? 'vr' : 'players'; const es = document.querySelectorAll('.room-detail-flipper[data-players][data-vr]'); for (const e of es) e.style.opacity = 0; setTimeout(() => { for (const e of es) { e.innerHTML = (ROOM_DETAIL_STATE === 'players') ? e.dataset.players : e.dataset.vr; e.style.opacity = 1; } }, 300); }; u(); ROOM_DETAIL_FLIPPER_TIMER = setInterval(u, 3000);
}
...
function remove_expired_mii_images(){ let r=[]; for(let i=0; i<localStorage.length; i++){ const k=localStorage.key(i); if(k.startsWith("mii_img_")){ try { const d=JSON.parse(localStorage.getItem(k)); if(!Array.isArray(d)||d.length<2||typeof d[1]!=='number'||(Date.now()-d[1]>MII_EXPIRE_TIME)) r.push(k); } catch(e){ r.push(k); } } } for(const k of r) localStorage.removeItem(k); }
function get_cached_mii_image(k){ const c=`mii_img_${k}`; const s=localStorage.getItem(c); if(!s) return null; try { const a=JSON.parse(s); if(!Array.isArray(a)||a.length<2||typeof a[1]!=='number'||(Date.now()-a[1]>MII_EXPIRE_TIME)){ localStorage.removeItem(c); return null; } return a[0]; } catch(e){ localStorage.removeItem(c); return null; } }
function set_cached_mii_image(k, img, rawDataRef = null){ if (!k || !img) return; const c = `mii_img_${k}`; const finalImg = format_mii_src(img); const d = [finalImg, Date.now(), rawDataRef]; try { localStorage.setItem(c, JSON.stringify(d)); } catch(e) { if (e.name === 'QuotaExceededError') { remove_expired_mii_images(); try { localStorage.setItem(c, JSON.stringify(d)); } catch (e2) {} } } }
...
function apply_mii_image(k, img){ const els=document.querySelectorAll(`img[mii-data="${CSS.escape(k)}"]`); if(!els||els.length===0) return; let src = format_mii_src(img); for(const el of els) { if (el.src !== src) el.src=src; } }

async function fetch_mii_images(keys){ let u=[...new Set(keys)].filter(Boolean), f=[]; for(const k of u){ let c=get_cached_mii_image(k); if(c) apply_mii_image(k, c); else f.push(k); } for(let i=0; i<f.length; i+=MAX_MIIS_PER_REQUEST){ const b=f.slice(i, i+MAX_MIIS_PER_REQUEST); if(b.length>0){ try { const r=await fetch(MII_API_URL,{method:"POST",headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}); if(!r.ok) throw new Error(`Mii API: ${r.status}`); const d=await r.json(); for(const k of Object.keys(d)){ if(d[k]) { apply_mii_image(k, d[k]); set_cached_mii_image(k, d[k]); } } } catch(e){ console.error("Mii fetch batch error:", e); } } } }
async function fetch_miis_by_fc(fcs){ let u=[...new Set(fcs)].filter(Boolean), f=[]; for(const k of u){ let c=get_cached_mii_image(k); if(c) apply_mii_image(k, c); else f.push(k); } for(let i=0; i<f.length; i+=MAX_MIIS_PER_REQUEST){ const b=f.slice(i, i+MAX_MIIS_PER_REQUEST); if(b.length>0){ try{ const payload = { friendCodes: b }; const r=await fetch(MII_BATCH_API_URL,{method:"POST",headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); if(!r.ok) throw new Error(`Mii Batch: ${r.status}`); const d=await r.json(); if (d.miis) { for(const k of Object.keys(d.miis)){ if(d.miis[k]){ apply_mii_image(k, d.miis[k]); set_cached_mii_image(k, d.miis[k]); } } } } catch(e){ console.error("Mii batch fetch error:", e); } } } }
...
async function on_load(){
    ...
    if(ts){ 
        ...
        el("history-input").value = histIso; 
...
async function fetch_rooms() {
    ...
    try { 
        ...
        if(HISTORY_MODE){ 
            if(rd&&typeof rd.data !=='undefined'&&typeof rd.timestamp !=='undefined'){ dt=new Date(rd.timestamp*1000); rms=rd.data; let z=new Date().getTimezoneOffset()*60000; let localDate = new Date(rd.timestamp*1000 - z); 
            const isoStr = localDate.toISOString().slice(0, 16);
            el("history-input").value = isoStr; 
            if(el("history-input-main")) el("history-input-main").value = isoStr;
        ...
        if (HISTORY_MODE) { let zo=HISTORY_DATE.getTimezoneOffset()*60000; let lt=new Date(HISTORY_DATE.getTime()-zo); let hin=el("history-input"); if(hin && !isNaN(lt.getTime())){ 
            const val = lt.toISOString().slice(0,16);
            hin.value = val; 
            if(el("history-input-main")) el("history-input-main").value = val;
        } }
...
async function reload_rooms(){
     ...
     for(const room of rooms){
         ...
         if (room.type === "anybody" && (!rk || rk === "-1")) { rk_h = `<span style="color:#FFF">Public</span>`; rk_t = "Public Room"; } else if(ti){ rk_h=`<span title="${rk||'Unknown'}" style="color: ${ti.c};">${ti.l}</span>`; rk_t=ti.l; } else if(rk){ rk_h=rk.toUpperCase(); rk_t=rk.toUpperCase(); }
         ...
         for(const p_idx of sk){
             ...
             for(let cmi=0; cmi<nm; cmi++){
                 ...
                 if(p.fc && mk) {
                    const fcKey = `mii_img_${p.fc}`;
                    const storedItem = localStorage.getItem(fcKey);
                    let needsUpdate = false;
                    if(storedItem) { try { const parsed = JSON.parse(storedItem); if(parsed.length > 2 && parsed[2] !== mk) { needsUpdate = true; } else if (parsed.length <= 2) { needsUpdate = true; } } catch(e){ needsUpdate = true; } }
                    if(needsUpdate) { const rawImg = get_cached_mii_image(mk); if(rawImg) { set_cached_mii_image(p.fc, rawImg, mk); } else if (storedItem) { localStorage.removeItem(fcKey); } }
                 }

                 let cached = get_cached_mii_image(mk); 
                 if (!cached && p.fc) { cached = get_cached_mii_image(p.fc); }
                 ...
                 if (!isGuest) { 
                     ...
                     if (isJoining) { pv.innerHTML = '<span class="player-info-loading-text">Joining...</span>'; } else if(p.ev && p.eb){ pv.dataset.vr=p.ev; pv.dataset.br=p.eb; pv.textContent = (VRBR_STATE === 'vr') ? `${p.ev} VR` : `${p.eb} BR`; } else { pv.textContent="🚫 VR/BR"; } 
                     pr.appendChild(pv); 
                 }
         ...
         if (total_vr > 0) { let vrHTML = `🏆 • <span class='room-player-count excited'>${avg_vr}</span> VR Avg.`; det.innerHTML = `<span class="room-detail-flipper" data-players="${detailHTML}" data-vr="${vrHTML}">${detailHTML}</span>`; } else { det.innerHTML = `<span>${detailHTML}</span>`; }
